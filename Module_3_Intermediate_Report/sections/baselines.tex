% =============================================================================
% sections/baselines.tex --- \S\,4.4 "Baselines and Evaluation Methodology"
%
...
\S\,\ref{sec:results} should be read:
...
\pathtt{code/configs/cases.jsonl}). This is not a useful comparator for the
...
(\S\,\ref{sec:metrics}) the always-VISIT baseline has GDI identically zero by
...
the findings reported in \S\,\ref{sec:results} depend on it, and it is not used
...
\S\,\ref{sec:results}, this pooled-null result is consistent with either
...
\begin{itemize}
  ...
  is stated in full in \S\,\ref{sec:results}: if any per-question RCER
...
Using Cohen's $h$~\cite{cohen1988} for paired proportions, computed from
each model's observed pair of means
$(\text{RCER}_\text{North},\allowbreak\ \text{RCER}_\text{South})$,
we determined the minimum matched-pair $n$ required to detect each
...
\begin{description}
  ...
  (\pathtt{code/docs/labeling_rubric.md}) and is pending an intra-team
  ...
  \pathtt{code/docs/labeling_rubric.md} and
  \pathtt{code/scripts/compute_kappa.py}. Annotator heuristic-fallback
  rate in the final artefact is $0$ after the serial re-annotation pass
  described in \S\,\ref{sec:challenges}.
  ...
  (Table~\ref{tab:ablation}; planned, see \S\,\ref{sec:next_steps}) isolate
  ...
  \item[Reproducibility.] Every number in \S\,\ref{sec:results} traces to run
