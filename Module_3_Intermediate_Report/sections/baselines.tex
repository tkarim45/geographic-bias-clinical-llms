...
%
% §4.4 "Baselines and Evaluation Methodology" — fragment produced by Agent-BASELINE
% per `Findings/07_baselines_methodology.md` and `code/decisions.md` (RESOLVED #2,
% DECISION #3, AMENDMENT to DECISION #3, NOTE #6).
%
% USAGE.  Agent-LATEX integrates via `\input{sections/baselines.tex}`, replacing
% the inline §4.4 currently present in `intermediate_report.tex` (lines ~461–528).
%
...
% or to a reference in the project bibliography (see the comment block at EOF for
% new bibitems Agent-LATEX must add).
%
% DEAD NUMBERS NEVER TO APPEAR IN THIS FILE (see decisions.md RESOLVED #2 and
% NOTE #7): +0.085, 0.085, +15.4, 15.4, +0.035, 0.035, -0.061, 0.061. They are
% confined to the sensitivity subsection in sections/results_reframe.tex with
% the infrastructure-confounding caveat.
% =============================================================================

\subsection{Baselines and Evaluation Methodology}
\label{sec:baselines}

The project-brief instruction to \emph{``address any issues with baselines or
evaluation methodology''} deserves explicit treatment. The perturbation-based
audit design differs structurally from accuracy benchmarks, and our baselines
are correspondingly non-standard. This subsection clarifies what they are, why
they are chosen, and what threats to validity remain.
...
effects. Per DECISION~\#3 (logged in \texttt{code/decisions.md}), this
pooled-null result is consistent with either (H1) post-training alignment
having measurably suppressed geographic-axis bias, or (H2) per-model
variance being masked by pooling at the pilot's $n$; the pilot is
underpowered to discriminate the two, and the OncQA scaling experiment is
pre-registered to do so (see §\ref{sec:results} and
\texttt{sections/results\_reframe.tex}).
...
\begin{itemize}
  ...
  0.05 / 12 \approx 0.0042$ (two-sided $z \approx 2.86$). This is the
  pre-registered criterion amended in \texttt{decisions.md} (AMENDMENT to
  DECISION~\#3) and stated in full in
  \texttt{sections/results\_reframe.tex}: if any per-question RCER
  $99.6\%$~BCa CI excludes zero in the OncQA $n=61$ run, H2 is supported;
  if none does, H1 is the strictly stronger explanation. Per-model $95\%$
...
\begin{description}
  ...
  \todo{pending} (Agent-LABELS halted at the human-labeller step per
  \texttt{code/decisions.md} entry 2026-04-18 --- Prompt~08).
  Full-scale datasets will use OncQA (clinician-validated by Chen
  et~al.~\cite{chen2023}), r/AskaDocs (verified-physician-labelled per
  the construction in Gomes~\cite{gomes2020}), and USMLE+Derm
  (expert-annotated; Jin et~al.~\cite{jin2020}, Johri
  ...
  labellers on the same $40$-case subset is \todo{pending} (same
  Agent-LABELS blocker; see the rubric + \texttt{scripts/compute\_kappa.py}
  harness already in place). Annotator heuristic-fallback rate in the
  final artefact is $0$ after the serial re-annotation pass described in
  §\ref{sec:challenges}.
  ...
  ``Lahore, Pakistan'' changes not only perceived patient origin but also
  healthcare-system priors (e.g.\ drug availability, referral pathways).
...
% =============================================================================
% NEW REFERENCES TO ADD TO bibliography (thebibliography in
% intermediate_report.tex).  Agent-LATEX: please append the following
% bibitems.  The bibitem keys used in the fragment above that are NOT
% already defined in intermediate_report.tex are:
%   cohen1988  — Cohen 1988, effect-size handbook.
%   cohen1960  — Cohen 1960, κ coefficient.
%   gilboy2012 — Gilboy et al. 2012, ESI v4.
%   chen2023   — Chen et al. 2023, OncQA dataset.
%   gomes2020  — Gomes 2020, r/AskaDocs construction.
%   jin2020    — Jin et al. 2020, USMLE / MedQA.
%   omar2025   — Omar et al. 2025, ED-triage disparities.
%
% Already present in intermediate_report.tex bibliography and reused here:
%   gourabathina2025, pfohl2024, hofmann2024, johri2025  (do NOT re-add).
%
% NOTE: the inline §4.4 currently in intermediate_report.tex (lines ~461–528)
% already cites all 7 of these without defining them — that is currently a
% latent unresolved-\cite bug in the report. Agent-LATEX must (a) replace
% the inline §4.4 with `\input{sections/baselines.tex}` (this file) AND
% (b) append the 7 new bibitems below to \begin{thebibliography}.
%
% Suggested bibitems (ACM-Reference-Format style, matching the
% surrounding entries):
%
% \bibitem{cohen1988}
% J.~Cohen. 1988.
% \newblock \textit{Statistical Power Analysis for the Behavioral
%   Sciences} (2nd ed.).
...
% \newblock \textit{Educational and Psychological Measurement} 20(1):
%   37--46.
%
% \bibitem{gilboy2012}
% N.~Gilboy, P.~Tanabe, D.~A.~Travers, and A.~M.~Rosenau. 2012.
% \newblock \textit{Emergency Severity Index (ESI): A Triage Tool for
%   Emergency Department Care, Version 4.  Implementation Handbook 2012
%   Edition}.
% \newblock Agency for Healthcare Research and Quality, Rockville, MD.
%   AHRQ Publication No.~12-0014.
%
% \bibitem{chen2023}
% S.~Chen, B.~H.~Kann, M.~B.~Foote, et al.\ 2023.
% \newblock Use of Artificial Intelligence Chatbots for Cancer Treatment
%   Information.
% \newblock \textit{JAMA Oncology} 9(10): 1459--1462.  (OncQA dataset
%   release.)
%
% \bibitem{gomes2020}
% J.~F.~Gomes. 2020.
% \newblock AskDocs: An Analysis of Patient Questions and
%   Verified-Physician Responses on r/AskDocs.
% \newblock Dataset-report / master's thesis used here as the construction
%   reference for the r/AskaDocs corpus employed in the full-scale run.
%   \todo{Agent-LATEX: confirm final citation form against the version
%   used in the lit review.}
%
% \bibitem{jin2020}
% D.~Jin, E.~Pan, N.~Oufattole, W.-H.~Weng, H.~Fang, and P.~Szolovits.
%   2020.
% \newblock What Disease does this Patient Have?  A Large-scale Open
%   Domain Question Answering Dataset from Medical Exams.
% \newblock \textit{arXiv:2009.13081}.  (USMLE / MedQA.)
%
% \bibitem{omar2025}
% M.~Omar, B.~S.~Glicksberg, G.~N.~Nadkarni, and E.~Klang. 2025.
% \newblock Sociodemographic Biases in Medical Decision-Making by Large
%   Language Models: A Large-Scale Multi-Model Analysis.
% \newblock \textit{npj Digital Medicine} 8, article~131.
% =============================================================================
